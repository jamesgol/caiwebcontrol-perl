...
function ajaxCommand(url, container, repeat, data)
{
	url += "?rand=" + Math.random(); 	// append a random number to url query string to prevent local browser cache
	newAJAXCommand(url, container, repeat, data);
}
	...
	for(i = ajaxList.length; i > 0; i--)
	{
		...
		if(curAjax.ajaxReq.readyState == 4 && curAjax.ajaxReq.status == 200) {
			...
			if(typeof(curAjax.container) == 'function')
			{
				if(window.DOMParser)
				{
					...
					xmlDoc = parser.parseFromString(curAjax.ajaxReq.responseText,"text/xml");
					curAjax.container(xmlDoc);
				}
				else
				{
					/* microsoft ! */
				  ...
				  xmlDoc.async="false";
				  xmlDoc.loadXML(curAjax.ajaxReq.responseText); 
				  curAjax.container(xmlDoc);
				}
		...
		if(elapsed > timeOutMS) {
			...
			if(typeof(curAjax.container) == 'function'){
				...
				// Alert the user
				//alert("Command failed.\nConnection to NetDev was lost.");
			}

	    	curAjax.ajaxReq.abort();
	    	curAjax.ajaxReq = null;
			
			// If it's a repeatable request, then do so only if it did not fail
			/*if(curAjax.repeat)
				newAJAXCommand(curAjax.url, curAjax.container, curAjax.repeat); */
			continue;
		}
...
function getXMLValue(xmlData, field)
{
	...
	{
		var str = "";
		var len = xmlData.getElementsByTagName(field)[0].childNodes.length; 
		for(var i = 0; i<len; i++)
		{
			str += xmlData.getElementsByTagName(field)[0].childNodes[i].nodeValue;
		}
